...
# Training parameters.
parser.add_argument('--data_dir', '-dd', type=str, default='data/bkai')
...
if __name__ == "__main__":
    ...
    pred_slots, pred_intents = process.predict(
    ...
    with open('results.csv', 'w', encoding="utf-8") as f:
        for pred_intent, pred_slot in zip(pred_intents, pred_slots):
            line = ""
            for slot in pred_slot:
                line = line + slot + " "
            pred_intent_vote = Counter(pred_intent).most_common(1)[0][0]
            f.write("{}, {}\n".format(pred_intent_vote, line.strip()))
