...
class TorchDataset(Dataset):
    ...
    def __len__(self):
        ...
        print(len(self.__text), len(self.__slot))
...
class DatasetManager(object):
    ...
    def quick_build(self):
        ...
        self.add_file(self.__args.data_dir, 'train', if_train_file=True)
        self.add_file(self.__args.data_dir, 'dev', if_train_file=False)
        self.add_file(self.__args.data_dir, 'test', if_train_file=False)
    ...
    def add_file(self, file_path, data_name, if_train_file):
        text, slot, intent = self.__read_file(file_path, data_name)
    ...
    @staticmethod
    def __read_file(file_path, data_name):
        ...
        with open(file_path + "/" + data_name + "/seq.in", 'r') as f:
            texts = [line.strip().split() for line in f.readlines()]

        if os.path.exists(file_path + "/" + data_name + "/seq.out"):
            with open(file_path + "/" + data_name + "/seq.out", 'r') as f:
                slots = [line.strip().split() for line in f.readlines()]
        else:
            # temp slots for test
            slots = [['O']*len(line) for line in texts]
        if os.path.exists(file_path + "/" + data_name + "/label"):
            with open(file_path + "/" + data_name + "/label", 'r') as f:
                intents = [line.strip().split() for line in f.readlines()]
        else:
            # temp slots for test
            intents = ['greeting' for _ in texts]
