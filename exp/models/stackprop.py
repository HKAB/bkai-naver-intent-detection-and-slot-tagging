...
class StackPropagation(nn.Module):
    ...
    def __init__(self, pretrained_model, hidden_dim, num_intent, num_slot, dropout = 0.2, max_len = None, reverse = False) -> None:
        super().__init__()
        ...
        if not reverse:
            ...
            intent_input_dim = config.hidden_size + num_slot
            self.intent_dec = LSTMDecoder(intent_input_dim, hidden_dim, num_intent, dropout = dropout, max_len = max_len, use_crf = False)
            self.slot_dec = LSTMDecoder(config.hidden_size, hidden_dim, num_slot, dropout = dropout, max_len = max_len)
    ...
    def forward(self, input_ids, att_mask, len_list):
        ...
        if not self.reverse:
            ...
            slot_pred = self.slot_dec(embedding, len_list)
            slot_prob = torch.softmax(slot_pred, dim = -1)
            intent_inputs = torch.cat([embedding, slot_prob], dim = -1)
            intent_pred = self.intent_dec(intent_inputs, len_list)
    ...
    def get_intent(self, seq_pred, mask, slot_out):
        intent_pred = []
        # print(mask, seq_pred)
        for sent_pred, sent_mask, slot in zip(seq_pred, mask, slot_out):
            true_sent = [label for label, m, s in zip(sent_pred, sent_mask, slot) if m and s != 0] # non O slot tokens
            if len(true_sent) > 0:
                intent_pred.append(stats.mode(true_sent)[0][0])
            else:
                intent_pred.append(5) # greeting
    ...
    def predict(self, text, att_mask, slots, len_list, device, perm_idx = None, intents = None, return_logits = False):
        ...
        seq_mask = slots >= 0
        intent_out = self.get_intent(intent_logits, seq_mask, slot_out).to(device)
        if return_logits:
...
class LowDim(nn.Module):
    ...
    def __init__(self, pretrained_model, hidden_dim, num_intent, num_slot, dropout = 0.2, max_len = None) -> None:
        ...
        config = XLMRobertaConfig.from_pretrained(pretrained_model)
        self.xlmr = XLMRobertaModel.from_pretrained(pretrained_model)
        # self.intent_dec = nn.Linear(num_slot, num_intent)
        self.intent_dec = LSTMDecoder(num_slot, num_slot, num_intent, dropout = dropout, max_len = max_len, use_crf = False)
        self.intent_crf = CRF(num_intent, batch_first = True)
        self.slot_dec = LSTMDecoder(config.hidden_size, hidden_dim, num_slot, dropout = dropout, max_len = max_len)
    ...
    def forward(self, input_ids, att_mask, len_list):
        ...
        slot_pred = self.slot_dec(embedding, len_list)
        slot_prob = torch.softmax(slot_pred, dim = -1)
        intent_pred = self.intent_dec(slot_prob, len_list)
    ...
    def predict(self, text, att_mask, slots, len_list, device, perm_idx = None, intents = None):
        intent_out, slot_out = self.forward(text, att_mask, len_list)
        intent_out = self.intent_crf.decode(intent_out)
        seq_mask = slots >= 0
