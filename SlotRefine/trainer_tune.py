...
class Trainer(tune.Trainable):
    ...
    def setup(self, config):
        ...
        self.dev_dataset = config['dev_dataset']
        ...
        self.model_config = self.config_class.from_pretrained(self.args.model_name_or_path, finetuning_task=self.args.task)
        ...
        self.model.set_loss_coef(config["intent_loss_coef"], config["slot_loss_coef"])
        
        train_sampler = RandomSampler(self.train_dataset)
        self.train_dataloader = DataLoader(self.train_dataset, sampler=train_sampler, batch_size=config['batch_size'])
        ...
        self.optimizer = AdamW(optimizer_grouped_parameters, lr=config['lr'], eps=self.args.adam_epsilon)
        self.scheduler = get_linear_schedule_with_warmup(self.optimizer, num_warmup_steps=self.args.warmup_steps, num_training_steps=t_total)
    ...
    def step(self):
        ...
        return {"sementic_frame_acc": results['sementic_frame_acc'],
                "intent_acc": results['intent_acc'],
                "slot_precision": results['slot_precision']
                }
